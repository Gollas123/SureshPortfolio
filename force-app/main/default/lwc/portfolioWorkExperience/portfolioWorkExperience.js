...
export default class PortfolioWorkExperience extends LightningElement {
    ...
    formatExperience(data){
       this.workExperienceList = [...data.records].reverse().map(item=>{
            let id = item.id;
            const {JobStartDate__c, JobEndDate__c, 
                   Role__c, CompanyName__c, WorkLocation__c, 
            ...
            return {id, JobEndDate, JobStartDate, CompanyName, Role, WorkLocation, Description, IsCurrent}
